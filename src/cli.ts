...
function run(proc: NodeJS.Process) {
  ...
  program
    .version("1.0.0")
    .name("bank-schema-cli")
    .usage("--fileType <fileType> --filePath <filePath>")
    .option(
      "-t, --fileType <fileType>",
      "The type of input file. Valid file types are: " + fileTypes.join(" ")
    )
    .option(
      "-f, --filePath <filePath>",
  ...
  if (!program.fileType) {
    console.error(
      "You must specify a file type. Type --help for more details."
    );
    proc.exit(1);
  }
  ...
  if (!program.filePath) {
    console.error(
      "You must specify a file path. Type --help for more details."
    );
    proc.exit(1);
  }
  ...
  try {
    ...
    parseFromFile({ fileType: params.fileType, filePath: params.filePath })
      .then(printJson)
      .catch(console.error);
  } catch (e) {
